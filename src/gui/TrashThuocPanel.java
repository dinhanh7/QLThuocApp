...
public class TrashThuocPanel extends JPanel {
    ...
    public TrashThuocPanel(JTabbedPane tabbedPane, int index) {
        ...
        searchLabel.setIcon(new ImageIcon(TrashThuocPanel.class.getResource("/icon/chungSearch.png")));
        ...
        // Bảng
        String[] columnNames = {"", "ID Thuốc", "Tên thuốc", "Giá nhập", "Hạn sử dụng", "Số lượng", "Xuất xứ"};
        ...
        btnRefresh.setIcon(new ImageIcon(TrashThuocPanel.class.getResource("/icon/chungRefresh.png")));
        JButton btnRestore = new JButton("Khôi phục");
        btnRestore.setIcon(new ImageIcon(TrashThuocPanel.class.getResource("/icon/ChungUndo.png")));
        JButton btnDeleteForever = new JButton("Xóa vĩnh viễn");
        btnDeleteForever.setIcon(new ImageIcon(TrashThuocPanel.class.getResource("/icon/chungTrash.png")));
    ...
    void loadData() {
        model.setRowCount(0);
        List<Thuoc> list = dao.getDeleted();
        for (Thuoc t : list) {
        ...
        if (parentTabbedPane != null && tabIndex < parentTabbedPane.getTabCount()) {
            parentTabbedPane.setTitleAt(tabIndex, "Thuốc (" + getDeletedCount() + ")");
        }
    }
    ...
    private void restoreSelectedRows() {
        ...
        if (ids.isEmpty()) {
            JOptionPane.showMessageDialog(this, "Vui lòng chọn thuốc để khôi phục");
            return;
        }
    ...
    private void deleteSelectedRows() {
        ...
        int confirm = JOptionPane.showConfirmDialog(this, "Xóa vĩnh viễn các thuốc đã chọn?", "Xác nhận", JOptionPane.YES_NO_OPTION);
