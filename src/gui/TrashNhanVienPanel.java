...
public class TrashNhanVienPanel extends JPanel {
    ...
    public TrashNhanVienPanel(JTabbedPane tabbedPane, int index) {
        ...
        String[] columnNames = {"", "ID Nhân viên", "Họ tên","SĐT", "Giới tính", "Năm sinh", "Ngày vào làm",  "Lương", "Trạng thái"};
        ...
        btnRefresh.setIcon(new ImageIcon(TrashNhanVienPanel.class.getResource("/icon/chungRefresh.png")));
        JButton btnRestore = new JButton("Khôi phục");
        btnRestore.setIcon(new ImageIcon(TrashNhanVienPanel.class.getResource("/icon/ChungUndo.png")));
        JButton btnDeleteForever = new JButton("Xóa vĩnh viễn");
        btnDeleteForever.setIcon(new ImageIcon(TrashNhanVienPanel.class.getResource("/icon/chungTrash.png")));
    ...
    void loadData() {
        model.setRowCount(0);
        List<NhanVien> list = dao.getDeleted();
        ...
        if (parentTabbedPane != null && tabIndex < parentTabbedPane.getTabCount()) {
            parentTabbedPane.setTitleAt(tabIndex, "Nhân viên (" + getDeletedCount() + ")");
        }
    }
    ...
    private void restoreSelectedRows() {
        ...
        if (ids.isEmpty()) {
            JOptionPane.showMessageDialog(this, "Vui lòng chọn nhân viên để khôi phục");
            return;
        }
    ...
    private void deleteSelectedRows() {
        ...
        int confirm = JOptionPane.showConfirmDialog(this, "Xóa vĩnh viễn nhân viên đã chọn?", "Xác nhận", JOptionPane.YES_NO_OPTION);
